...
def main(args):
  ...
  for i in xrange(len(cam1_images)):
    cam1_pid = int(osp.basename(cam1_images[i])[:3])
    cam2_pid = int(osp.basename(cam2_images[i])[:3])
    assert cam1_pid == cam2_pid
    p_id = len(identities)
    p_images = []
    # view-0
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      'viper_dir',
      help="Root directory of the VIPeR dataset containing cam_a/ and cam_b/")
  parser.add_argument(
      'output_dir',
