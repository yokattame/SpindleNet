...
def main(args):
  try:
    ...
    matdata = loadmat(osp.join(args.cuhk03_dir, 'cuhk-03.mat'))
  except:
  ...
  for imgs_labeled, imgs_detected in zip(
    ...
    # We merge the manually labeled and automatically detected images of the same view.
    for i in xrange(imgs_labeled.shape[0]):
      ...
      for j in xrange(5):
        if imgs_labeled[i, j].size == 0:
          break
        file_name = 'cam_0/{:05d}_{:05d}.bmp'.format(pid, len(v_images))
        imsave(osp.join(args.output_dir, file_name), imgs_labeled[i, j])
        v_images.append(file_name)
      for j in xrange(5):
        if imgs_detected[i, j].size == 0:
          break
        file_name = 'cam_0/{:05d}_{:05d}.bmp'.format(pid, len(v_images))
        imsave(osp.join(args.output_dir, file_name), imgs_detected[i, j])
        v_images.append(file_name)
      ...
      for j in xrange(5, 10):
        if imgs_labeled[i, j].size == 0:
          break
        file_name = 'cam_1/{:05d}_{:05d}.bmp'.format(pid, len(v_images))
        imsave(osp.join(args.output_dir, file_name), imgs_labeled[i, j])
        v_images.append(file_name)
      for j in xrange(5, 10):
        if imgs_detected[i, j].size == 0:
